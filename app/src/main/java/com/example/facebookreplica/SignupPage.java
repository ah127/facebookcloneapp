...
public class SignupPage extends AppCompatActivity implements View.OnClickListener, RadioGroup.OnCheckedChangeListener {
    ...
    private void register() {
        ...
        RetrofitApi api = com.example.facebookreplica.network.Retrofit.retrofitInit().create(RetrofitApi.class);
        ...
        registerresponse.enqueue(new Callback<List<Response>>() {
            ...
            public void onResponse(Call<List<Response>> call, retrofit2.Response<List<Response>> response) {
                LoginPage.userlist = new ArrayList<>();
                if (response.isSuccessful()) {
                    regres = response.body();
                    LoginPage.userlist.add(regres.get(0).getUser());
//                    Log.d(TAG, "onResponse: " + userlist.get(0).getEmail());
                    edit.putString(Constants.TOKEN, response.body().get(0).getToken());
                    edit.putString(Constants.EMAIL, LoginPage.userlist.get(0).getEmail());
                    edit.commit();
                    ...
                    Toast.makeText(SignupPage.this, "Error While Registering", Toast.LENGTH_SHORT).show();
                }
            }
